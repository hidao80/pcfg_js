 ...
 * Editing by hidao on 21/06/27.
 */
///<reference path="../../node_modules/@types/kuromoji/index.d.ts" />

import kuromojin = require("kuromojin");
...
class Pcfg {
    ...
    static parse(
        ...
        var parser = new Pcfg();
        var fn = function (tokens: Array<any>) {
        ...
        parser.tokenize(text, verbose, fn);
    }

    tokenize(
        text: string,
        verbose: boolean,
        callback: (tokens: Array<any>) => void
    ): void {
        kuromojin.tokenize(text).then((parsed) => {
            parsed.forEach((token: any, i: Number) => {
                if (verbose)
                    console.log(
                        token.surface_form + " " + Pcfg.getJoinedPos(token)
                    );
            });
            callback(parsed);
        });
    ...
    calc(tokens: Array<any>): boolean {
            ...
            for (var i = 0; i < N - n; i++) {
                ...
                for (var j = 1; j < 1 + n; j++) {
                    for (
                        ...
                        //CYK
                        var left =
                            this.nodeMap[x][x + (j - 1)].inside[rule.result1];
                        if (!left) {
                            left = 0;
                        }
                        var bottom =
                            this.nodeMap[x + j][y].inside[rule.result2];
                        if (!bottom) {
                            bottom = 0;
                        }
                        if (!this.nodeMap[x][y]) {
                        ...
                        this.nodeMap[x][y].inside[rule.source] =
                            old + rule.probability * left * bottom;
                        ...
                        if (rule.probability * left * bottom > 0) {
                            ...
                            if (!this.ruleTreeMap[x][y].rules[rule.source]) {
                                this.ruleTreeMap[x][y].rules[rule.source] = <
                                    Rule[]
                                >[rule];
                            } else {
                                this.ruleTreeMap[x][y].rules[rule.source].push(
                                    rule
                                );
                            }
                            this.ruleTreeMap[x][y].left[rule.toString()] = {
                                x: x,
                                y: x + (j - 1),
                            };
                            this.ruleTreeMap[x][y].right[rule.toString()] = {
                                x: x + j,
                                y: y,
                            };
                        }
        ...
        for (var i = 0; i < N; i++) {
            for (var n = 0; n < N - i; n++) {
                for (var source in Pcfg.toUniqueArray(
                    this.rules.map((x) => x.source)
                )) {
                ...
                for (
                    ...
                    if (this.nodeMap[x][y].outside[rule.source] > 0) {
                        for (var k = 1; k < 1 + n; k++) {
                            //下側
                            if (
                                this.nodeMap[x + k][y].inside[rule.result2] > 0
                            ) {
                                this.nodeMap[x + k][y].outside[rule.result2] =
                                    this.nodeMap[x + k][y].inside[
                                        rule.result2
                                    ] +
                                    rule.probability *
                                        this.nodeMap[x][y].outside[
                                            rule.source
                                        ] *
                                        this.nodeMap[x][x - (1 - k)].inside[
                                            rule.result1
                                        ];
                            }
                            //左側
                            if (
                                this.nodeMap[x][y - k].inside[rule.result1] > 0
                            ) {
                                this.nodeMap[x][y - k].outside[rule.result1] =
                                    this.nodeMap[x][y - k].outside[
                                        rule.result1
                                    ] +
                                    rule.probability *
                                        this.nodeMap[x][y].outside[
    ...
    recalcProbability(tokens: Array<any>, nodeMap: PcfgNode[][]) {
        for (var i = 0; i < this.rules.length; i++) {
            var rule = this.rules[i];
            var newP =
                Pcfg.usedCount(tokens.length, rule, nodeMap) /
                this.rules
                    .filter((x) => x.source === rule.source)
                    .reduce(
                        (pv, x) =>
                            pv + Pcfg.usedCount(tokens.length, x, nodeMap),
                        0
                    );
